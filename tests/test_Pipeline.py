...
class TestPipeline(unittest.TestCase):
    def setUp(self):
        ...
        self.good_config_file = join(package_root, "tests", "configuration.json")
        ...
        self.bad_sample_sheet_path = self.path("duplicate_sample-sample-sheet.csv")
    ...
    def create_runinfo_file(self, four_reads=False):
        ...
        copy(join("tests/data/", f_name), self.runinfo_file)
    ...
    def test_make_sif_fname(self):
        ...
        obs = Pipeline.make_sif_fname("211021_A00000_0000_SAMPLE", "StudyA_13059")
        self.assertEqual(exp, obs)

    def test_is_sif_fp(self):
        obs1 = Pipeline.is_sif_fp(
            "/path/to/sifs/211021_A00000_0000_SAMPLE_StudyA_13059_blanks.tsv"
        )
        self.assertTrue(obs1)

        obs2 = Pipeline.is_sif_fp(
            "/path/to/sifs/211021_A00000_0000_SAMPLE_StudyA_13059_lord_of_the.sif"
        )
    ...
    def test_get_orig_names_from_sheet_with_replicates(self):
        ...
        exp = {
            ...
            "JBI.KHP.HGL.023",
            "JBI.KHP.HGL.024",
            "RMA.KHP.rpoS.Mage.Q97D",
            "RMA.KHP.rpoS.Mage.Q97E",
            "RMA.KHP.rpoS.Mage.Q97L",
            "RMA.KHP.rpoS.Mage.Q97N",
        }
    ...
    def test_creation(self):
        ...
        self.assertEqual(
            msg, f"'search_paths' is not a key in {self.path()}/bad_configuration.json"
        )
        ...
        self.assertEqual(
            str(e.exception), "does/not/exist/configuration.json does not exist."
        )
        ...
        self.assertEqual(str(e.exception), "configuration_file_path cannot be None")
        ...
        self.assertEqual(
            str(e.exception),
            "A run-dir for 'not-sample-sequence-directory' could not be found",
        )
        ...
        with self.assertRaisesRegex(
            PipelineError, "good-sample-sheet.csv is not a valid json file"
        ):
            Pipeline(
                self.good_sample_sheet_path,
                self.good_run_id,
        ...
        with open(bad_json_file, "w") as f:
            f.write(
                '{ "not_profile": { "instrument_type": "default", '
        ...
        with open(bad_json_file, "w") as f:
            f.write(
                '{ "profile": { "not_instrument_type": "default", '
                '"configuration": { "bcl2fastq": { "nodes": 1, "nprocs": '
                '16, "queue": "qiita", "wallclock_time_in_minutes": 216, '
                '"modules_to_load": [ "bcl2fastq_2.20.0.422" ], '
                '"executable_path": "bcl2fastq", '
    ...
    def test_sample_sheet_validation(self):
        ...
        try:
            ...
            self.fail(
                (
                    "test_filter_directories_for_time failed w/PipelineEr"
                    f"ror: {e.message}"
                )
            )
    ...
    def test_generate_sample_information_files(self):
        ...
        paths = pipeline.generate_sample_info_files([self.good_sample_sheet_path])
        exp = [
            (f"{self.path()}/output_dir/{self.good_run_id}_StudyA_13059_blanks.tsv"),
            (f"{self.path()}/output_dir/{self.good_run_id}_StudyB_11661_blanks.tsv"),
            (f"{self.path()}/output_dir/{self.good_run_id}_StudyC_6123_blanks.tsv"),
        ]
        ...
        exp_lines = {
            f"{self.good_run_id}_StudyA_13059_blanks.tsv": 33,
            f"{self.good_run_id}_StudyB_11661_blanks.tsv": 8,
            f"{self.good_run_id}_StudyC_6123_blanks.tsv": 2,
        }

        exp_first_lines = {
            f"{self.good_run_id}_StudyA_13059_blanks.tsv": "BLANK1.1A\t2017-05-23\t193\t"
            ...
            "StudyA\tTRUE\t"
            "UCSD\tFALSE",
            f"{self.good_run_id}_StudyB_11661_blanks.tsv": "BLANK.40.12G\t2017-05-23\t193\tControl"
            ...
            f"{self.good_run_id}_StudyC_6123_blanks.tsv": "BLANK.41.12G\t2017-05-23\t193\tControl"
            "\tNegative\tSterile water blank\tSterile water blank\turban"
        ...
        exp_last_lines = {
            f"{self.good_run_id}_StudyA_13059_blanks.tsv": "BLANK4.4H\t2017-05-23\t193\t"
            "Control\tNegative\tSterile w"
            "ater blank\tSterile water blank\turban biome\tres"
            "earch facility\tsterile wate"
            "r\tmisc environment\tUSA:CA:"
            ...
            "StudyA\tTRUE\t"
            "UCSD\tFALSE",
            f"{self.good_run_id}_StudyB_11661_blanks.tsv": "BLANK.43.12H\t2017-05-23\t193\tControl"
            "\tNegative\tSterile water blank\tSterile water blank\turban"
            " biome\tresearch facility\tsterile wat"
            "er\tmisc environment\tUSA:CA:San Diego"
            ...
            f"{self.good_run_id}_StudyC_6123_blanks.tsv": "BLANK.41.12G\t2017-05-23\t193\tContro"
    ...
    def _help_test_generate_sample_information_files_with_multiple_preps(
        ...
        sample_sheet_paths = [
            sample_sheet_path.replace(".csv", f"_demux_{x}.csv") for x in range(1, 4)
        ]
        ...
        for curr_sif_path in sif_paths:
            ...
            with open(curr_sif_path, "r") as f:
                ...
                self.assertEqual(set(curr_sample_names), set(curr_expected_blanks))
    ...
    def test_generate_sample_information_files_multiple_preps_no_context(self):
        ...
        expected_blanks_by_qiita_id = {"11661": ["BLANK.43.12G", "BLANK.43.12H"]}
    ...
    def test_get_sample_ids(self):
        exp_sample_ids = [
            "CDPH-SAL__Salmonella__Typhi__MDL-143",
    ...
    def test_get_sample_names(self):
        exp_sample_ids = [
            "CDPH-SAL..Salmonella..Typhi..MDL-143",
            ...
            "JM-Metabolic.GN0.2005",
            "JM-Metabolic.GN0.2007",
            "JM-Metabolic.GN0.2009",
            "JM-Metabolic.GN0.2094",
            "JM-Metabolic.GN0.2099",
            "JM-Metabolic.GN0.2148",
            "JM-Metabolic.GN0.2165",
            "JM-Metabolic.GN0.2169",
            "JM-Metabolic.GN0.2172",
            "JM-Metabolic.GN0.2175",
            "JM-Metabolic.GN0.2183",
            "JM-Metabolic.GN0.2215",
            "JM-Metabolic.GN0.2254",
            "JM-Metabolic.GN0.2277",
            "JM-Metabolic.GN0.2290",
            "JM-Metabolic.GN0.2337",
            "JM-Metabolic.GN0.2317",
            "JM-Metabolic.GN0.2354",
            "JM-Metabolic.GN0.2375",
            "JM-Metabolic.GN0.2380",
            "JM-Metabolic.GN0.2393",
            "JM-Metabolic.GN0.2404",
            "5B",
            "6A",
            "BLANK.41.12G",
            "BLANK.41.12H",
            ...
            "JM-Metabolic.GN02514",
            "JM-Metabolic.GN02529",
            "JM-Metabolic.GN02531",
            "JM-Metabolic.GN02567",
            "JM-Metabolic.GN02590",
            "JM-Metabolic.GN02657",
            "JM-Metabolic.GN02748",
            "JM-Metabolic.GN02766",
            "JM-Metabolic.GN02769",
            "JM-Metabolic.GN02787",
            "JM-Metabolic.GN03132",
            "JM-Metabolic.GN03218",
            "JM-Metabolic.GN03252",
            "JM-Metabolic.GN03409",
            "JM-Metabolic.GN04014",
            "JM-Metabolic.GN04094",
            "JM-Metabolic.GN04255",
            "JM-Metabolic.GN04306",
            "JM-Metabolic.GN04428",
            "JM-Metabolic.GN04488",
            "JM-Metabolic.GN04540",
            "JM-Metabolic.GN04563",
            "JM-Metabolic.GN04612",
            "JM-Metabolic.GN04665",
            "JM-Metabolic.GN04682",
            "JM-Metabolic.GN05002",
            "JM-Metabolic.GN05109",
            "JM-Metabolic.GN05128",
            "JM-Metabolic.GN05367",
            "JM-Metabolic.GN05377",
            "7A",
            "8A",
            "BLANK.42.12G",
            "BLANK.42.12H",
            ...
            "JM-Metabolic.GN02424",
            "JM-Metabolic.GN02446",
            "JM-Metabolic.GN02449",
            "JM-Metabolic.GN02487",
            "JM-Metabolic.GN02501",
            "ISB",
            "GFR",
            "BLANK.43.12G",
            "BLANK.43.12H",
        ...
        exp = {"3A", "4A", "5B", "6A", "BLANK.41.12G", "7A", "8A", "ISB", "GFR"}
    ...
    def test_get_project_info(self):
        ...
        exp_project_names = ["StudyA_13059", "StudyB_11661", "StudyC_6123"]
    ...
    def test_parse_project_name(self):
        ...
        tests = {
            ...
            "False": [
                ("ProjectN_13059", ("ProjectN_13059", "13059")),
    ...
    def test_identify_reserved_words(self):
        ...
        obs = pipeline.identify_reserved_words(["NOT_A_RESERVED_WORD", "ANOTHER_WORD"])
        self.assertEqual(obs, [])

        # assert that 'well_id_384' is a reserved word.
        obs = pipeline.identify_reserved_words(["sample_well", "NOT_A_RESERVED_WORD"])
        ...
        obs = pipeline.identify_reserved_words(
...
class TestAmpliconPipeline(unittest.TestCase):
    ...
    def test_mapping_file_validation(self):
        ...
        self.assertEqual(
            str(e.exception),
            ("Mapping-file is missing columns: tm10_8_tool, tm50_8_tool, well_id_384"),
        )
        ...
        self.assertEqual(
            str(e.exception),
            ("Mapping-file contains duplicate columns: ['barcode', 'Barcode']"),
        )
    ...
    def test_generate_sample_information_files(self):
        ...
        paths = pipeline.generate_sample_info_files([self.good_mapping_file_path])
    ...
    def test_dummy_sheet_generation(self):
        ...
        dummy_sheet_path = join(self.output_file_path, "dummy_sample_sheet.csv")
    ...
    def test_process_run_info_file(self):
        ...
        exp = [
            {"NumCycles": 151, "Number": 1, "IsIndexedRead": False},
        ...
        with self.assertRaisesRegex(ValueError, "Cannot extract read information"):
    ...
    def test_identify_reserved_words(self):
        ...
        # also reserved.
        obs = pipeline.identify_reserved_words(
            ["Sample_Well", "TM300_8_Tool", "tm300_8_tool", "well_id_384"]
        )
...
class TestInstrumentUtils(unittest.TestCase):
    ...
    def test_instrument_utils(self):
        ...
        exp = {
            "231108_M04586_0992_000000000-L7342": {
                "id": "M04586",
                "type": "MiSeq",
                "date": "2023-11-08",
            },
            ...
            "20220912_FS10001773_27_BSE39218-1017": {
                "id": "FS10001773",
                "type": "iSeq",
                "date": "2022-09-12",
            },
        ...
        for run_id, run_dir in run_directories:
            self.assertEqual(iutils.get_instrument_type(run_dir), exp[run_id]["type"])
            self.assertEqual(iutils._get_date(run_dir), exp[run_id]["date"])
...
good_dummy_sheet1 = [
    ...
    ",,,,,,",
    "[Settings],,,,,,",
    "OverrideCycles,Y151;N8;N8;Y151,,,,,",
    "MaskShortReads,1,,,,,",
    ...
    ",,,,,,",
    "[Bioinformatics],,,,,,",
    ("Project,ForwardAdapter,ReverseAdapter,HumanFiltering,QiitaID,,"),
