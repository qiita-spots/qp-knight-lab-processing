...
class TestConvertJob(unittest.TestCase):
    def setUp(self):
        ...
        self.StudyC_ids = ['7A', 'ISB', '5B', '6A', '8A', '3A', 'GFR', '4A',
        ...
        file_names = [join(fastq_base,
                           'StudyB_11661',
                           '%s_R1.fastq.gz' % x) for x in self.StudyB_ids]
        file_names += [join(fastq_base,
                            'StudyB_11661',
                            '%s_R2.fastq.gz' % x) for x in self.StudyB_ids]
        file_names += [join(fastq_base,
                            'StudyC_6123',
                            '%s_R1.fastq.gz' % x) for x in self.StudyC_ids]
        file_names += [join(fastq_base,
                            'StudyC_6123',
                            '%s_R2.fastq.gz' % x) for x in self.StudyC_ids]
        file_names += [join(fastq_base,
                            'StudyA_13059',
                            '%s_R1.fastq.gz' % x) for
                       x in self.nyu_ids]
    ...
    def test_copy_sequences_bad_parameters(self):
        ...
        with self.assertRaisesRegex(ValueError, "source 'StudyB_11661' and "
    ...
    def test_copy_sequences_success_w_replicates(self):
        ...
        files_to_match = [fp.replace('StudyB_11661',
                                     'StudyA_13059')
