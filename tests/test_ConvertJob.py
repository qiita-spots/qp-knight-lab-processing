...
class TestConvertJob(unittest.TestCase):
    def setUp(self):
        ...
        makedirs(join(fastq_base, "StudyB_11661"), exist_ok=True)
        makedirs(join(fastq_base, "StudyC_6123"), exist_ok=True)
        makedirs(join(fastq_base, "StudyA_13059"), exist_ok=True)

        # generate filenames and paths for the dummy fastq files.
        file_names = [
            join(fastq_base, "StudyB_11661", "%s_R1.fastq.gz" % x)
            for x in self.StudyB_ids
        ]
        file_names += [
            join(fastq_base, "StudyB_11661", "%s_R2.fastq.gz" % x)
            for x in self.StudyB_ids
        ]
        file_names += [
            join(fastq_base, "StudyC_6123", "%s_R1.fastq.gz" % x)
            for x in self.StudyC_ids
        ]
        file_names += [
            join(fastq_base, "StudyC_6123", "%s_R2.fastq.gz" % x)
            for x in self.StudyC_ids
        ]
        file_names += [
            join(fastq_base, "StudyA_13059", "%s_R1.fastq.gz" % x) for x in self.nyu_ids
        ]
        file_names += [
            join(fastq_base, "StudyA_13059", "%s_R2.fastq.gz" % x) for x in self.nyu_ids
        ]
        ...
        with open(self.convert_log_path, "w") as f:
            f.write(
                "2024-01-01T12:12:12Z thread 99999 ERROR: Sample Sheet "
    ...
    def test_creation(self):
        ...
        with open(join(self.good_output_path, "ConvertJob", "ConvertJob.sh")) as f:
    ...
    def test_parse_sample_sheet(self):
        ...
        exp = {
            "run_directory": ("/sequencing/igm_runs/210820_A00953_0380_BHJ53TDSX2"),
            "sample_sheet_path": (
                "/qmounts/qiita_data/working_dir/"
    ...
    def test_copy_sequences_bad_parameters(self):
        ...
        source_project = "StudyB_11661"
        other_projects = ["StudyA_13059", "StudyC_6123"]
        ...
        err_msg = (
            "'NOT_A_SAMPLE_NAME' did not match any 'sample_name' values"
            " in project 'StudyB_11661'."
        )
        with self.assertRaisesRegex(ValueError, err_msg):
            job.copy_sequences(not_a_sample_name, source_project, dest_project)

        err_msg = (
            "'CDPH-SAL.Salmonella.Typhi.MDL-154' did not match any "
            "'sample_name' values in project 'StudyC_6123'."
        )
        with self.assertRaisesRegex(ValueError, err_msg):
            job.copy_sequences(sample_name, not_source_project, dest_project)

        with self.assertRaisesRegex(
            ValueError, "'NOT_A_PROJECT' is not defined in sample sheet"
        ):
            job.copy_sequences(sample_name, not_a_project, dest_project)

        with self.assertRaisesRegex(
            ValueError, "'NOT_A_PROJECT' is not defined in sample sheet"
        ):
            job.copy_sequences(sample_name, source_project, not_a_project)

        with self.assertRaisesRegex(
            ValueError,
            "source 'StudyB_11661' and "
            "destination 'StudyB_11661' "
            "projects are the same",
        ):
            job.copy_sequences(sample_name, source_project, source_project)
    ...
    def test_copy_sequences_success(self):
        ...
        sample_name = "CDPH-SAL.Salmonella.Typhi.MDL-154"
        source_project = "StudyB_11661"
        dest_project = "StudyA_13059"
        projects = ["StudyA_13059", "StudyC_6123", "StudyB_11661"]
        ...
        # fake a fastq file in the 'StudyB_11661' directory for the purposes of
        # copying it into the 'StudyA_13059' project.
        with open(
            join(
                self.good_output_path,
                "ConvertJob",
                source_project,
                "CDPH-SAL_Salmonella_Typhi_MDL-154_S1_L001_R1_001.fastq.gz",
            ),
            "w",
        ) as f:
            f.write("Hello World!\n")
        ...
        # 'CDPH-SAL.Salmonella.Typhi.MDL-154' from 'StudyB_11661' to
        # 'StudyA_13059'. 'StudyC_6123' should remain empty; the
        ...
        dst_file = source_file.replace("StudyB_11661", "StudyA_13059")
        self.assertTrue(exists(dst_file))

        # file should not have been copied here.
        dst_file = source_file.replace("StudyB_11661", "StudyC_6123")
    ...
    def test_copy_sequences_success_w_replicates(self):
        ...
        source_project = "StudyB_11661"
        dest_project = "StudyA_13059"
        projects = ["StudyA_13059", "StudyB_11661"]
        ...
        fastqs = [
            "RMA_KHP_rpoS_Mage_Q97D_A5_S1_L001_R1_001.fastq.gz",
        ...
        for fastq in fastqs:
            with open(
                join(self.good_output_path, "ConvertJob", source_project, fastq), "w"
            ) as f:
                f.write("Hello World!\n")
        ...
        files_to_match = [
            fp.replace("StudyB_11661", "StudyA_13059") for fp in files_to_match
        ]
    ...
    def test_copy_controls_between_projects(self):
        ...
        job = ConvertJob(
            run_dir,
            self.good_output_path,
            self.sample_sheet_w_context_path,
            "qiita",
            1,
            16,
            1440,
        ...
        for some_project in projects:
            ...
            # project defined in the sample-sheet.
            makedirs(
                join(self.good_output_path, "ConvertJob", some_project), exist_ok=True
            )
