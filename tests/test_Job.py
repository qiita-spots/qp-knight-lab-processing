...
from os.path import abspath, dirname, exists, isdir, join, split
...
class TestJob(unittest.TestCase):
    ...
    def test_system_call(self):
        ...
        job = Job(
            self.path("211021_A00000_0000_SAMPLE"),
            output_dir,
        ...
        self.assertRegex(str(e.exception), r"^file '/does/not/exist' does not exist.")

        obs = job._find_files(package_root)
        obs = [
            re.sub(
                r"^.*?/sequence_processing_pipeline", r"sequence_processing_pipeline", x
            )
            for x in obs
        ]
        ...
        obs = job._system_call(
            "ls " + join(package_root, "tests", "bin"), callback=my_callback
        )
        ...
        self.assertRegex(
            str(e.exception),
            (
                r"^Execute command-line statement"
                r" failure:\nCommand: "
                r"error\nreturn code:"
                r" 127"
            ),
    ...
    def test_extract_project_names_from_fastq_dir(self):
        ...
        self.remove_these.append(base_path("7b9d7d9c-2cd4-4d54-94ac-40e07a713585"))

        for fastq in dummy_fastqs:
            full_path = base_path("7b9d7d9c-2cd4-4d54-94ac-40e07a713585", fastq)
        ...
        job = Job(
            base_path("211021_A00000_0000_SAMPLE"),
    ...
    def test_query_slurm(self):
        ...
        self.remove_these.append(join(package_root, "tests", "bin", "my_state.json"))
