...
from os import W_OK, access, chmod, environ, getcwd, makedirs, remove, walk
...
class TestWorkflows(TestCase):
    ...
    def _generate_empty_file_cmd(self, file_path, size):
        ...
        if type in types:
            return (
                f"dd if=/dev/zero of={file_path} bs={size}{types[type]} "
                "count=1 2>/dev/null"
            )
        else:
    ...
    def test_partial_metagenomic_pipeline(self):
        ...
        sheet = load_sample_sheet(
            "tests/data/sample-sheets/metagenomic/illumina/good_sheet1.csv"
        )
        ...
        exp = [
            "#!/bin/bash",
            "#SBATCH --job-name 077c4da8-74eb-4184-8860-0207f53623be_ConvertJob",  # noqa
            "#SBATCH -p qiita",
            "#SBATCH -N 1",
            "#SBATCH -n 16",
            "#SBATCH --time 216",
        ...
        with open(
            "tests/data/077c4da8-74eb-4184-8860-0207f53623be/ConvertJob/ConvertJob.sh",  # noqa
            "r",
        ) as f:
            obs = f.readlines()
        ...
        cmds.append(
            "mkdir -p %s"
            % join(self.output_dir, "NuQCJob", "fastp_reports_dir", "html")
        )
        cmds.append(
            "mkdir -p %s"
            % join(self.output_dir, "NuQCJob", "fastp_reports_dir", "json")
        )
    ...
    def test_partial_metatranscriptomic_pipeline(self):
        ...
        # fake fastq files for convert_raw_to_fastq() to find and manipulate.
        sheet = load_sample_sheet(
            "tests/data/sample-sheets/metatranscriptomic/illumina/good_sheet1.csv"  # noqa
        )
        ...
        # the audit results match what is expected.
        kwargs = {
            "uif_path": "tests/data/sample-sheets/metatranscript"
    ...
    def test_partial_amplicon_pipeline(self):
        ...
        r1 = join(
            join(self.output_dir, "ConvertJob"),
    ...
    def test_partial_tellseq_pipeline(self):
        ...
        for file_type in ["I1", "R1", "R2"]:
            for x in barcode_ids:
                files.append(f"project_{file_type}_{x}_raw.fastq.gz")
        ...
        files = []
        for file_type in ["I1", "R1", "R2"]:
            files += [f"project_{file_type}_{x}.fastq.gz" for x in barcode_ids]
        ...
        self.create_fake_bin("squeue.2", "echo 'JOBID,STATE\n9999991,COMPLETED'")  # noqa

        kwargs = {
            "uif_path": (
                "tests/data/sample-sheets/metagenomic/tellseq/good_sheet1.csv"
            ),
            "qclient": FakeClient(),
            "lane_number": "4",
            "config_fp": "tests/configuration.json",
