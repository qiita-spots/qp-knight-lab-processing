    ...
    def __init__(self, **kwargs):
        ...
        if "update_qiita" in kwargs:
            if not isinstance(kwargs["update_qiita"], bool):
                msg = "value for 'update_qiita' must be of type bool"
...
class PrepNuQCWorkflow(StandardMetagenomicWorkflow):
    def __init__(self, **kwargs):
        ...
        if html_summary is None:
            raise WorkflowError(
                f"Artifact {aid} does not have a summary; please generate one."
            )
        ...
        for fs in files.values():
            for f in fs:
                ...
                bn = (
                    basename(f["filepath"])
                    .replace(".trimmed.fastq.gz", ".fastq.gz")[:-9]
                    .replace(".", "_")
                    + ".fastq.gz"
                )
                ...
                if fformat is None:
                    ...
                    fformat = [
                        fn
                        for fn, rxs in FILES_REGEX.items()
        ...
        for i, (sn, vals) in enumerate(iter(pt.iterrows())):
            ...
            _d = df_summary[df_summary.filename.str.startswith(rp)]
            if _d.shape[0] != 2:
                mlen = _d.shape[0]
                ValueError(f"The run_prefix {rp} from {k} has {mlen} matches")
        ...
        sheet = make_sample_sheet(metadata, data_df, "NovaSeqX", [1])
