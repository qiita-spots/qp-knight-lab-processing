...
def sequence_processing_pipeline(qclient, job_id, parameters, out_dir):
    ...
    if is_restart:
        ...
        with open(out_path("restart_me"), "r") as f:
            ...
            if len(lines) != 2:
                raise ValueError(
                    f"{out_path('restart_me')} can only contain "
        ...
        if (
            ...
            and sheet.Header["Chemistry"] == "Amplicon"
        ):
        ...
        if {"body", "content_type", "filename"} != set(user_input_file):
            return (
                ...
                None,
                (
                    "This doesn't appear to be a valid sample "
    ...
    return (True, [ArtifactInfo("output", "job-output-folder", paths)], status_line.msg)
