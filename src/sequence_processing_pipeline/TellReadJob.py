...
class TellReadJob(Job):
    def __init__(
        ...
        """
        super().__init__(
            run_dir, output_path, "TellReadJob", [], 1, modules_to_load=modules_to_load
        )
    ...
    def _process_sample_sheet(self):
        ...
        for sample in sheet.samples:
            sample_ids.append(
                (sample["Sample_ID"], sample["Sample_Project"], sample["barcode_id"])
            )
    ...
    def audit(self):
        ...
        for root, dirs, files in walk(join(self.output_path, "Full")):
            for _file in files:
                m = re.match(
                    r"TellReadJob_(.\d)_(C\d\d\d).fastq.gz.corrected."
                    r"err_barcode_removed.fastq",
                    _file,
                )
                if m:
