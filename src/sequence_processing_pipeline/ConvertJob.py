...
class ConvertJob(Job):
    def __init__(
        ...
        if not tmp:
            msg = f"{self.bcl_tool} is not the path to a knownexecutable"
    ...
    def _generate_job_script(self):
        ...
        if "bcl-convert" in self.bcl_tool:
            lines.append(
                ...
                % (self.bcl_tool, self.sample_sheet_path, self.output_path)
            )
    ...
    def run(self, callback=None):
        ...
        """
        try:
            jsp = self.job_script_path
            lp = self.log_path
            job_info = self.submit_job(jsp, exec_from=lp, callback=callback)
    ...
    def parse_job_script(job_script_path):
        ...
        if m:
            ...
            msg = f"could not detect run_directory in '{job_script_path}'"
        ...
        if m:
            ...
            raise ValueError(
                f"could not detect sample-sheet path in '{job_script_path}'"
            )
    ...
    def copy_controls_between_projects(self):
        ...
        for curr_control in self.controls_info.values():
            ...
            curr_primary_qiita_study_name = _get_proj_name(
                self.info, curr_primary_qiita_study_id
            )

            csqs_ids = curr_control[SECONDARY_STUDIES_KEY]
            for curr_second_qiita_study_id in csqs_ids:
                curr_secondary_qiita_study_name = _get_proj_name(
                    self.info, curr_second_qiita_study_id
                )
                self.copy_sequences(
                    curr_sample_name,
    ...
    def _get_sample_sheet_info(self):
        ...
        for curr_project_name, curr_project_info in projects_info.items():
            ...
            cpn_path = join(self.output_path, curr_project_name)
            fastq_paths = self._find_files(cpn_path)
            fastq_paths = [f for f in fastq_paths if f.endswith(".fastq.gz")]

            _vals = curr_project_info[SAMPLES_DETAILS_KEY].values()
            for curr_sample_info in _vals:
                ...
                rgx = (
                    r"^"
                    + re.escape(curr_sample_info[SS_SAMPLE_ID_KEY])
    ...
    def copy_sequences(self, sample_name, source_project, dest_project):
        ...
        if self.info is None:
            raise ValueError(
                "This method cannot be called until processing has completed."
            )
        ...
        if source_project == dest_project:
            raise ValueError(
                f"source '{source_project}' and destination "
        ...
        if len(results) == 0:
            ...
            def _make_msg_base(col_name):
                return (
                    f"'{sample_name}' did not match any '{col_name}' "
            ...
            if self.contains_replicates:
                ...
                msg = _make_msg_base(ORIG_NAME_KEY) + (
                    f" Value '{sample_name}' may have a well-id appended to it"
                )
            else:
...
class ConvertPacBioBam2FastqJob(Job):
    ...
    def _generate_job_script(self):
        ...
        lines.append(
            ...
            f" | parallel --halt now,fail=1 -j {parallel_jobs}"
        )
