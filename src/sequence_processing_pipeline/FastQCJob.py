...
class FastQCJob(Job):
    ...
    def _find_projects(self, path_to_run_id_data_fastq_dir, is_raw_input):
        ...
        for directory in listdir(path_to_run_id_data_fastq_dir):
            ...
            files = [
                x for x in files if x.endswith(".fastq.gz") and "zero_files" not in x
            ]
            ...
            if r1_only:
                ...
                results.append((directory, filter_type, project_dir, r1_only, r2_only))
    ...
    def _scan_fastq_files(self, is_raw_input=False):
        find_path = (
            ...
            if is_raw_input
            else self.processed_fastq_files_path
        )
        ...
        if projects is None:
            raise PipelineError(
                "There are no fastq files for FastQCJob to process in %s." % find_path
            )
    ...
    def _get_failed_indexes(self, job_id):
        ...
        log_fp = join(self.output_path, "logs", f"failed_indexes_{job_id}.json")
