...
class SeqCountsJob(Job):
    ...
    def run(self, callback=None):
        ...
        params = ["--parsable", f"-J {self.job_name}", f"--array 1-{self.file_count}"]
        try:
    ...
    def parse_logs(self):
        ...
        for some_file in files:
            with open(some_file, "r") as f:
                msgs += [
                    line for line in f.readlines() if line.startswith("[E::stk_size]")
                ]
        ...
        for root, dirs, files in walk(self.log_path):
            for _file in files:
                if _file.endswith(".out"):
                    ...
                    _dir, _file, seq_counts, base_pairs = extract_metadata(
                        log_output_file
                    )
    ...
    def _aggregate_counts(self, sample_sheet_path):
        ...
        def get_metadata(sample_sheet_path):
            ...
            if len(lanes) != 1:
                raise ValueError("More than one lane is declared in sample-sheet")
        ...
        file_names = [
            x for x in list(by_files.keys()) if determine_orientation(x) in ["R1", "R2"]
        ]
        ...
        for sample_id in sample_ids:
            ...
            if len(found) != 2:
                ...
                raise ValueError(
                    f"Multiple file matches for sample-id '{sample_id}' found: {found}"
                )
